...
public interface DailyInventoryRepository extends JpaRepository<DailyInventory, Short> {
    ...
    @Query(value = "SELECT updated_at, sum(quantity) FROM daily_inventory d where d.branch_id = :branchId and d.updated_at between :startDate and :endDate group by updated_at", nativeQuery = true)
