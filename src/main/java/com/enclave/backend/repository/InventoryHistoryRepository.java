...
public interface InventoryHistoryRepository extends JpaRepository<InventoryHistory, Short> {
    ...
    @Query(value = "SELECT created_at, sum(quantity) FROM inventory_history i where i.branch_id = :branchId and i.created_at between :startDate and :endDate group by created_at order by created_at", nativeQuery = true)
